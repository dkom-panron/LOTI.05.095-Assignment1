...
class gauss_newton:
    def __init__(self, maxiter = 200, num_controls = 20):
        ...
        self.jac_func = jit(jacrev(self.compute_error_func, argnums = (0)))
    ...
    def compute_error_func(self, controls, 
        ...
        # error function is everything inside the squares
        #cost_goal_wpt = (x[-1]-x_goal)**2+(y[-1]-y_goal)**2
        error_goal_wpt = (x[-1]-x_goal) + (y[-1]-y_goal) # final waypoint

        #c_goal_traj = ((x-x_goal)**2+(y-y_goal)**2)
        #cost_goal_traj = jnp.sum(c_goal_traj)
        error_goal_traj = jnp.sum((x-x_goal) + (y-y_goal)) # entire trajectory
